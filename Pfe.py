...
img = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY) #Converstion de l'image en mode blanc et noir
...
def derive2x(mat):
    for i in range(0,m-1):      # Dérivée deuxiéme de x
        for j in range(0,m-1):
            d2x[i,j]=float(mat[i+1,j])+float(mat[i-1,j])-2*float(mat[i,j])
    return d2x
...
def derive2y(mat):
    for i in range(0,m-1):      # Dérivée deuxiéme de y
        for j in range(0,m-1):
            d2y[i,j]=float(mat[i,j+1])+float(mat[i,j-1])-2*float(mat[i,j])
    return d2y
...
def showimage():
    file=filedialog.askopenfilename(initialdir="/", title="choisir une image", filetypes=(("JPG files", ".jpg"),("PNG files", ".png"),("All files", ".")))
